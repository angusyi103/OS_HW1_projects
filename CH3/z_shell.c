...
int lsh_mkdir(char **args)
{
    if (args[1] == NULL)
    {
        fprintf(stderr, "mkdir: expected argement to \"mkdir\"\n");
    }
    else
    {
...
int lsh_rmdir(char **args)
{
    if (args[1] == NULL)
    {
        fprintf(stderr, "rmdir: expected argement to \"rmdir\"\n");
    }
    else
    {
        if (rmdir(args[1]) != 0)
...
int lsh_execute(char *line)
{
    ...
    // printf("%s\n", history[cur_pos]);
    // history[count] = '\0';
    if (cur_pos > -1)                              // changed to >-1
        printf("Inserted %s\n", history[cur_pos]); // changed cur_pos - 1 to cur_pos
